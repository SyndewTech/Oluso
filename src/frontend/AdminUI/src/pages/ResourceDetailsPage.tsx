...
import type { CreateResourceRequest, UpdateResourceRequest } from '../types/resources';
...
export default function ResourceDetailsPage() {
  ...
  const { data: resource, isLoading } = useQuery({
    queryKey: ['resource', numericId],
    queryFn: () => resourceService.getById(numericId),
    enabled: !isNew && !isNaN(numericId),
  });

  const { data: availableScopes = [] } = useQuery({
    queryKey: ['available-scopes'],
    queryFn: () => resourceService.getAvailableScopes(),
    enabled: !isNew,
  });
  ...
  const addScopeMutation = useMutation({
    mutationFn: (scopeName: string) => resourceService.addScope(numericId, scopeName),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['resource', numericId] });
  ...
  const removeScopeMutation = useMutation({
    mutationFn: (scopeName: string) => resourceService.removeScope(numericId, scopeName),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['resource', numericId] });
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data: UpdateResourceRequest) => resourceService.update(numericId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['resource', numericId] });
      queryClient.invalidateQueries({ queryKey: ['resources'] });
  ...
  const updateClaimsMutation = useMutation({
    mutationFn: (claims: string[]) => resourceService.update(numericId, { userClaims: claims }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['resource', numericId] });
  ...
  const validateUri = (uri: string): boolean => {
    setUriError(null);
    if (!uri) {
      setUriError('URI is required');
      return false;
    }
    try {
      const parsed = new URL(uri);
      if (!['http:', 'https:'].includes(parsed.protocol)) {
        setUriError('URI must use http or https protocol');
        return false;
      }
      if (parsed.hash) {
        setUriError('URI must not contain a fragment component (RFC 8707)');
        return false;
      }
      return true;
    } catch {
      setUriError('URI must be a valid absolute URI');
      return false;
    }
  };

  const handleCreate = () => {
    if (validateUri(formData.uri)) {
  ...
  const unassignedScopes = availableScopes?.filter(
    (scope) => !resource?.allowedScopes?.includes(scope.name)
  ) || [];
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Resource URI *</label>
              <input
                type="text"
                value={formData.uri}
                onChange={(e) => {
                ...
                onBlur={() => formData.uri && validateUri(formData.uri)}
              ...
              />
              {uriError ? (
                <p className="mt-1 text-xs text-red-600">{uriError}</p>
              ) : (
                <p className="mt-1 text-xs text-gray-500">
                  Absolute URI identifying this protected resource (RFC 8707). Must not contain a fragment.
                </p>
              )}
            </div>
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button onClick={handleCreate} disabled={!formData.uri || createMutation.isPending}>
  ...
  if (!resource) {
    return <div className="text-center py-8 text-gray-500">Resource not found</div>;
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{resource.displayName || 'Resource'}</h1>
            <p className="text-sm text-gray-500 break-all">{resource.uri}</p>
          </div>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <dl className="space-y-4">
              <div>
                <dt className="text-sm font-medium text-gray-500">Resource URI</dt>
                <dd className="mt-1 text-sm text-gray-900 break-all">{resource.uri}</dd>
              </div>
              <div>
            ...
            <dl className="space-y-4">
              ...
              {resource.updated && (
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">{new Date(resource.updated).toLocaleString()}</dd>
                </div>
              )}
            </dl>
          </CardContent>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            <p className="text-sm text-gray-500 mb-4">
              Scopes that are valid for this resource. If empty, all scopes are allowed.
              Clients can request these scopes when using the resource parameter.
            </p>
            {resource.allowedScopes && resource.allowedScopes.length > 0 ? (
              <Table
                columns={scopeColumns}
                data={resource.allowedScopes.map((s) => ({ name: s }))}
                keyExtractor={(item) => item.name}
              />
            ) : (
              <div className="text-center py-8 text-gray-500">
                No scope restrictions. All scopes are allowed for this resource.
              </div>
            )}
          </CardContent>
        </Card>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-500">
            Select a scope to add to this resource. Clients can then request this scope when using the resource parameter.
          </p>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            Are you sure you want to delete the resource <strong className="break-all">{resource.uri}</strong>?
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Resource URI</label>
            <p className="mt-1 text-sm text-gray-500 break-all">{resource.uri}</p>
            <p className="text-xs text-gray-400">URI cannot be changed after creation</p>
          </div>
